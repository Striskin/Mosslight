...
bool Region::solid(int x,int y) const { char t=tile(x,y); return t=='#'||t=='T'||t=='~'||t=='H'||t=='P'; }
Region loadRegion(const std::filesystem::path& data,RegionId id) {
    ...
    if(!(in>>tag>>r.width>>r.height>>r.palette>>std::quoted(r.name)>>std::quoted(r.subtitle))||
       tag!="MOSSMAP1"||r.width<30||r.width>128||r.height<17||r.height>128)
        throw std::runtime_error("Invalid map header: "+path.string());
    for(int y=0;y<r.height;++y) {
        std::string row; in>>row;
        if(static_cast<int>(row.size())!=r.width||row.find_first_not_of(".#T~HP,+_:*")!=std::string::npos)
    ...
    while(in>>tag) {
        if(tag=="exit") {
            ...
            if(!(in>>x>>y>>w>>h>>target>>sx>>sy>>lock)||target<0||target>=6||w<1||h<1)
            ...
            int type,x,y;
            if(!(in>>type>>x>>y)||type<0||type>2) throw std::runtime_error("Invalid enemy spawn");
            r.spawns.push_back({type,tileCenter(x,y)});
        } else if(tag=="object") {
            Object o; int kind,x,y,item;
            if(!(in>>kind>>o.id>>x>>y>>item>>o.amount>>std::quoted(o.label))||kind<0||kind>4||item<0||item>=ItemCount||o.amount<1||o.amount>99)
